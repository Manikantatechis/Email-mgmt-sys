...
const sendMessages = asyncHandler(async (req, res) => {
	...
	await saveBatch(batchId, userId, tableData.length);

	let smsSummary = [],
		emailSummary = [];

	if (actionType === "sms" || actionType === "both") {
		const kixieCredentials = await getDecryptedCredentials(KixieCredentials, actionData.kixieCredId);
		if(kixieCredentials.apiKey){
			kixieCredentials.apiKey = await decryptData(kixieCredentials.apiKey);
		}
		const kixieTemplate = await getTemplate(KixieTemplate, actionData.kixieTemplateId);
		smsSummary = await sendSms(kixieCredentials, kixieTemplate, tableData);
	}

	if (actionType === "email" || actionType === "both") {
		const gmailCredentials = await getDecryptedCredentials(GmailCredentials, actionData.emailCredId);
		if(gmailCredentials.oauthClientSecret && gmailCredentials.oauthRefreshToken){
			gmailCredentials.oauthClientSecret = await decryptData(gmailCredentials.oauthClientSecret);
			gmailCredentials.oauthRefreshToken = await decryptData(gmailCredentials.oauthRefreshToken);
		}
		const gmailTemplate = await getTemplate(GmailTemplate, actionData.emailTemplateId);
		emailSummary = await sendEmail(gmailCredentials, gmailTemplate, tableData, batchId);
	}

	const summary = { batchId, smsSummary, emailSummary };
...
function generateBatchId(userId) {
	...
	return crypto.createHash("sha256").update(`${userId}-${batchTime}`).digest("hex");
}

// Function to save a new batch
async function saveBatch(batchId, userId, emailCount) {
...
function validateActionData(actionType, actionData) {
	...
	} else if (actionType === "both" && (!emailCredId || !emailTemplateId || !kixieCredId || !kixieTemplateId)) {
